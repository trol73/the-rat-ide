...
class DefaultSourceTree extends JTree {
    ...
    private void maybeShowPopup(MouseEvent e) {
        if (e.isPopupTrigger()) {
            ...
            if (row > -1) {
                ...
                if (treeModel.isLeaf(node) && tagText != null && !tagText.contains("(0)") && !tagText.startsWith("<")) {
                    ...
                    item.setMnemonic(msg.getString("InsertTagMnemonic").charAt(0));
                    ...
                    item.setMnemonic(msg.getString("JumpToTagMnemonic").charAt(0));
    ...
    /**
     * Sets the icon to use for "root" nodes of source trees (typically representing the type of source file).
     *
     ...
     */
    private class Listener extends MouseAdapter implements ActionListener, TreeSelectionListener {
        ...
        private String mouseMovedTagEntry(ExtendedTagEntry entry) {
            // If we've already generated the (probably HTML) tool tip text, use it.
            if (entry.cachedToolTipText != null) {
                return entry.cachedToolTipText;
            }
            // Create the tool tip text (HTML, or just text if only a line number was found (such as #defines for C/C++).
            String text = entry.getPlainTextPattern();
            // If all we have is a line number, use the tag's element's name as the tooltip.
            if (text == null) {
                return entry.name.replaceAll("\t", " ");
            }
            ...
            if (plugin.getUseHTMLToolTips()) {
                ...
                int pos = SearchEngine.getNextMatchPos(text, textArea.getText(), true, true, false);
                if (pos > -1) {
                    try {
                ...
                if (object instanceof TagEntry entry) {
                    ...
                    if (pattern != null) {
                        ...
                        if (pos > -1) {
                            ...
                            UIManager.getLookAndFeel().provideErrorFeedback(DefaultSourceTree.this);
