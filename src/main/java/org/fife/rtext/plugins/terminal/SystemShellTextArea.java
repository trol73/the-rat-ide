...
class SystemShellTextArea extends ConsoleTextArea {
    ...
    private void handleCd(String text) {
        ...
        if (!m.matches()) {
            append(plugin.getString("Error.IncorrectParamCount", CD), STYLE_STDERR);
            appendPrompt();
            return;
        }
    ...
    private void handlePwd(String text) {
        if (!text.equals(PWD)) {
            append(plugin.getString("Error.IncorrectParamCount", PWD), STYLE_STDERR);
            appendPrompt();
            return;
        }
    ...
    protected void handleSubmit(String text) {
        if (plugin.getApplication().getOS() == OS.WINDOWS) {
            ...
            if (text.startsWith(CD + '\\')) {
                ...
                text = CD + " " + root.getAbsolutePath() + text.substring(CD.length() + 1);
            }
