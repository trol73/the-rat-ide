 ...
 * @version 1.0
 */
public class FolderProjectEntryTreeNode extends FileProjectEntryTreeNode implements PhysicalLocationTreeNode {
    ...
    private void addChildrenFilteredAndSorted(File[] files) {
        ...
        for (File f : files) {
            ...
            if (filterInfo != null && filterInfo.isAllowed(f, isDir)) {
                if (isDir)
                    dirList.add(f);
                else
                    fileList.add(f);
