...
public class Workspace implements ModelEntity {
    ...
    public static Workspace load(ProjectPlugin plugin, File file) throws IOException {
        ...
        for (int i = 0; i < projElemList.getLength(); i++) {
            ...
            String encoding = projElem.getAttribute("encoding");
            String type = projElem.getAttribute("type");
            String device = projElem.getAttribute("device");
            String mainFile = projElem.getAttribute("mainfile");
            project.setEncoding(encoding);
            project.setType(ProjectType.fromStr(type));
     ...
     */
    private static Document loadXmlRepresentation(File file) throws IOException {
        ...
        try {
            ...
            InputSource is = new InputSource(new UnicodeReader(new FileInputStream(file), "UTF-8"));
            ...
            String text = Messages.getString("ProjectPlugin.XmlError", file.getName(), desc);
     ...
     */
    private static void recursivelyAddChildren(Element parentElem, ProjectEntryParent parent) {
        ...
        for (int j = 0; j < childNodes.getLength(); j++) {
            ...
            if (childNode.getNodeType() == Node.ELEMENT_NODE) {
                ...
                if (ProjectEntry.DIR_PROJECT_ENTRY.equals(tag)) {
                    ...
                    FolderFilterInfo info = new FolderFilterInfo(displayedFiles, hiddenFiles, hiddenFolders);
    ...
    public boolean setName(String name) {
        ...
        if (tempFile.isFile()) {
            String msg = rtext.getString("FileAlreadyExists", tempFile.getAbsolutePath());
            String title = rtext.getString("ConfDialogTitle");
            int rc = JOptionPane.showConfirmDialog(rtext, msg, title,
                    JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
            if (rc != JOptionPane.YES_OPTION) {
                // Bail early, but don't give an error message by returning false
                return true;
            }
        }
    ...
    private static class Handler extends DefaultHandler {
        ...
        public InputSource resolveEntity(String publicID, String systemID) {
            return new InputSource(getClass().getResourceAsStream("rtext-workspace-1.0.dtd"));
        }
