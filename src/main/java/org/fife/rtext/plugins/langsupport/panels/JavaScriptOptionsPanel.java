 ...
 * @version 1.0
 */
public class JavaScriptOptionsPanel extends OptionsDialogPanel {
     ...
     */
    public JavaScriptOptionsPanel(RText app) {
        ...
        addChildPanel(new FoldingOnlyOptionsPanel(app, SyntaxConstants.SYNTAX_STYLE_JAVASCRIPT, false));
    ...
    private Box createAutoActivationPanel(ResourceBundle msg) {
        ...
        box.setBorder(new OptionPanelBorder(msg.getString("Options.General.AutoActivation")));
        ...
        if (o.isLeftToRight()) {
            temp.add(aaDelayLabel);
            temp.add(aaDelayField);
            temp.add(Box.createRigidArea(spacer));
            temp.add(aaJavaKeysLabel);
            temp.add(aaJavaKeysField);
            temp.add(Box.createRigidArea(d));
            temp.add(Box.createRigidArea(d));
            temp.add(Box.createRigidArea(spacer));
            temp.add(aaDocKeysLabel);
            temp.add(aaDocKeysField);
        } else {
            temp.add(aaDelayField);
            temp.add(aaDelayLabel);
            temp.add(Box.createRigidArea(spacer));
    ...
    private Box createGeneralPanel(ResourceBundle msg) {
        Box box = Box.createVerticalBox();
        box.setBorder(new OptionPanelBorder(msg.getString("Options.General.Section.General")));
    ...
    private Box createSyntaxCheckingEnginePanel(ResourceBundle msg) {
        ...
        Box box = Box.createVerticalBox();
        box.setBorder(new OptionPanelBorder(msg.getString("Options.JavaScript.SyntaxCheckingEngine")));
        ...
        jsHintDescLabel = new SelectableLabel(msg.getString("Options.JavaScript.JSHint.Desc"));
        addLeftAligned(box, jsHintDescLabel, 5, 20);

        jshintrcLabel = UIUtil.newLabel(msg, "Options.JavaScript.JSHint.JSHintrc");
    ...
    protected void doApplyImpl(Frame owner) {
        ...
        reparse |= jls.setErrorParser(rhinoRB.isSelected() ? JsErrorParser.RHINO : JsErrorParser.JSHINT);
    ...
    protected OptionsPanelCheckResult ensureValidInputsImpl() {
        ...
        if (jshintrcField.isEnabled()) {
            ...
            if (jshintrc != null && !jshintrc.isFile()) {
                String msg = Plugin.MSG.getString("Options.JavaScirpt.Error.JSHint");
