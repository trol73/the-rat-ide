...
public class PerlOptionsPanel extends OptionsDialogPanel {
     ...
     */
    public PerlOptionsPanel(RText app) {
        ...
        box.setBorder(new OptionPanelBorder(msg.getString("Options.Perl.Section.CodeCompletion")));
        cp.add(box);
        cp.add(Box.createVerticalStrut(5));
        ...
        box = Box.createVerticalBox();
        box.setBorder(new OptionPanelBorder(msg.getString("Options.Perl.Section.SyntaxChecking")));
        cp.add(box);
        ...
        box = Box.createVerticalBox();
        box.setBorder(new OptionPanelBorder(msg.getString("Options.Perl.Section.Perl5Lib")));
        cp.add(box);
        overridePerl5LibCB = createCB("OverridePerl5Lib");
        addLeftAligned(box, overridePerl5LibCB, 5);
        DefaultTableModel model = new DefaultTableModel(0, 1);
        perl5Table = new ModifiableTable(model, ModifiableTable.BOTTOM, ModifiableTable.ALL_BUTTONS);
    ...
    protected void doApplyImpl(Frame owner) {
        ...
        File newLoc = installLocField.getText().length() > 0 ? new File(installLocField.getText()) : null;
        PerlLanguageSupport.setPerlInstallLocation(newLoc);
        reunderline |= (oldLoc == null && newLoc != null) || (oldLoc != null && !oldLoc.equals(newLoc));
    ...
    private void setPerl5TableContents(String contents) {
        DefaultTableModel model = (DefaultTableModel) perl5Table.getTable().getModel();
        model.setRowCount(0);
     ...
     */
    private class Listener implements ActionListener, DocumentListener, ModifiableTableListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (enabledCB == source) {
                ...
                RDirectoryChooser chooser = new RDirectoryChooser(getOptionsDialog());
                ...
                File locFile = PerlLanguageSupport.getDefaultPerlInstallLocation();
                String origInstallLoc = locFile == null ? "" : locFile.getAbsolutePath();
                String installFieldText = installLocField.getText();
                boolean installLocFieldModified = !origInstallLoc.equals(installFieldText);
    ...
    private static class Perl5ItemDialog extends EscapableDialog
        ...
        Perl5ItemDialog(JDialog owner) {
            ...
            cancelButton = new JButton(Plugin.MSG.getString("Options.General.Cancel"));
