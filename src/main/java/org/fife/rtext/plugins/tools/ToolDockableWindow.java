...
public class ToolDockableWindow extends DockableWindow implements ProcessRunnerOutputListener {
    ...
    public void outputWritten(Process p, String output, boolean stdout) {
        Style style = textArea.getStyle(stdout ? OutputTextPane.STYLE_STDOUT : OutputTextPane.STYLE_STDERR);
        appendWithStyle(output, style); // thread safe Swing calls
    }


    @Override
    public void processCompleted(final Process p, final int rc, final Throwable e) {
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            if (e == null) {
                ...
                title = MessageFormat.format(title, tool.getName(), Integer.toString(rc), Float.toString(time));
                ...
                appendWithStyle(text, textArea.getStyle(OutputTextPane.STYLE_EXCEPTION));
            } else {
    ...
    public boolean startingTool(Tool tool) {
        ...
        title = MessageFormat.format(title, tool.getName(), new SimpleDateFormat().format(new Date()));
        setDockableWindowTitle(title);
