 ...
 * Currently, this class only checks for accessible* members in the current file only
 * (e.g. no members in super classes, no other classes on the classpath, etc.).
 * So naturally, there is a lot of room for improvement. IDE-style applications, for example, would want to check
...
class JavaLinkGenerator implements LinkGenerator {
    ...
    private final JavaLanguageSupport jls;
