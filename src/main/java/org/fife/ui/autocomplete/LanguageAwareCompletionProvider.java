...
public class LanguageAwareCompletionProvider extends CompletionProviderBase implements ToolTipSupplier {
    ...
    private CompletionProvider getProviderFor(JTextComponent comp) {
        ...
        if (curToken == null) { // At end of the line
            ...
            return switch (type) {
                case Token.ERROR_STRING_DOUBLE -> getStringCompletionProvider();
                case Token.COMMENT_EOL, Token.COMMENT_MULTILINE -> getCommentCompletionProvider();
                case Token.COMMENT_DOCUMENTATION -> getDocCommentCompletionProvider();
                default -> getDefaultCompletionProvider();
            };
        ...
        return switch (curToken.getType()) {
            case Token.LITERAL_STRING_DOUBLE_QUOTE, Token.ERROR_STRING_DOUBLE -> getStringCompletionProvider();
            case Token.COMMENT_EOL, Token.COMMENT_MULTILINE -> getCommentCompletionProvider();
            case Token.COMMENT_DOCUMENTATION -> getDocCommentCompletionProvider();
            case Token.NULL, Token.WHITESPACE, Token.IDENTIFIER, Token.VARIABLE, Token.PREPROCESSOR, Token.DATA_TYPE, Token.FUNCTION, Token.OPERATOR -> getDefaultCompletionProvider();
            default -> null;
        };
