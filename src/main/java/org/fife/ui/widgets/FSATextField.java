...
public class FSATextField extends JTextField implements ComponentListener, DocumentListener, ListSelectionListener {
        ...
        actionMap.put("OnEsc", new AbstractAction() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (root != null) {
                    ...
                    if (im != null && am != null) {
                        Object obj = im.get(KeyStroke.getKeyStroke(KeyEvent.VK_ESCAPE, 0));
                        if (obj != null) {
    ...
    private String updateComboBoxContents() {
        ...
        if (!t2.equals(directory)) {
            ...
            if (containedFiles != null) {
                ...
                if (num > 0) {
                    Arrays.sort(containedFiles, IGNORE_CASE ? String.CASE_INSENSITIVE_ORDER : null);
                }
            } else {
                num = 0;
            }
        ...
        if (num > 0) {
            ...
            long count = 0;
            int i = Arrays.binarySearch(containedFiles, fpFileName, IGNORE_CASE ? String.CASE_INSENSITIVE_ORDER : null);
    ...
    private void updatePopupWindowDimensions() {
        ...
        contentPane.setMinimumSize(new Dimension(size.width, contentPane.getMinimumSize().height));
        contentPane.setPreferredSize(new Dimension(size.width, contentPane.getPreferredSize().height));
        contentPane.setMaximumSize(new Dimension(size.width, contentPane.getMaximumSize().height));
    }
